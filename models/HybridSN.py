...
parser.add_argument(
    '-o', '--optimizer', dest='optimizer', default='adam', help="Name of optimizer.")
parser.add_argument(
...
parser.add_argument(
    '-vs', '--valid_split', type=float, dest='valid_split', default=0.9, help="Percentage of validation split.")
...
class MultiHeadSelfAttention(MultiHeadAttention):
    ...
    def forward(self, x):
        ...
        #print('After reshape and permute:', x.shape)
        Q = self.query(x)
        #print('After query:', Q.shape)
        K = self.key(x)
        #print('After key:', K.shape)
        A = self.softmax(torch.bmm(Q, K.permute(0, 2, 1)) / math.sqrt(c))  # [b, h*w, h*w]
        #print('After attention:', A.shape)
        V = self.value(x)
        #print('After value:', V.shape)
...
class HybridSN_network(nn.Module):
    def __init__(self, band, classes):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(
                in_channels=768,  # 展平后的尺寸为 32
                out_channels=64,
                kernel_size=(3, 3),
                stride=1,
                padding=(0, 1)),  # 保持宽度尺寸不变
            nn.ReLU()
        )
        ...
        self.dense1 = nn.Sequential(
            nn.Linear(2240, 256),  # 注意力机制后的输入尺寸
            nn.ReLU(),
            nn.Dropout(p=0.4)
        )
